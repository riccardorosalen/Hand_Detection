...
/*     std::string modelPath = "../SVM_model.txt";
    ...
    std::vector<std::vector<float>>images_pixel_acc;
    float true_hands_pixels_mean = 0;
    float true_background_pixels_mean = 0;
    for (int i = 1; i <=images_to_compute; i++) {        
        ...
        segmentation_masks.push_back(getSegmentation(images[i], cluster_centroids));
    ...
    for (int i = 1; i <=images_to_compute;i++) {
        images_pixel_acc.push_back(pixelAccuracyMetric(masks[i], segmentation_masks[i-1]));
        std::cout << "Image " << i << std::endl;   
        std::cout << "Pixel correctly classified as hands:" << images_pixel_acc[i-1][0]<<std::endl;
        std::cout << "Pixel correctly classified as background:" << images_pixel_acc[i-1][1] << std::endl;
        std::cout <<"Pixel accuracy:"<< (images_pixel_acc[i - 1][1]+ images_pixel_acc[i - 1][0])/2 << std::endl;
        true_hands_pixels_mean = true_hands_pixels_mean + images_pixel_acc[i-1][0];
        true_background_pixels_mean = true_background_pixels_mean + images_pixel_acc[i-1][1];
        cv::imshow("", images[i]);
        cv::waitKey(0);
    }

    std::cout << "Overall Performance"<< std::endl;
    std::cout << "Overall Pixel correctly classified as hands:" << true_hands_pixels_mean/images_to_compute << std::endl;
    std::cout << "Overall Pixel correctly classified as background:" << true_background_pixels_mean / images_to_compute<< std::endl;
    std::cout << "Overall Pixel accuracy:" << (true_background_pixels_mean+ true_hands_pixels_mean) / (images_to_compute *2)<< std::endl;
